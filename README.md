...
- **架构**：UI / 弹窗 / ViewModel / Controller / Service / Model / Storage / Config 分层，职责清晰、易于测试
...
├── config.py            # Config 层：文件路径、分类、日期格式等常量
├── tests/               # Tests 层：pytest 测试套件
│   ├── conftest.py          # 共享 fixture 与路径注入
│   ├── test_*.py            # models/storage/services/controller/view_models/dialogs/reports
│   └── stubs/               # 无 GUI 环境下使用的 tkinter / matplotlib 桩
├── README.md            # 项目说明
└── requirements.txt     # 第三方依赖
```

分层调用关系：`UI 层 → 弹窗层 → ViewModel 层 → Controller 层 → Service 层 / Model 层 → Storage 层`，`Config 层` 提供全局常量。
...
测试套件包含 50 个用例，覆盖 models / storage / services / controller / view_models / dialogs / reports 各层，测试在隔离的临时目录中运行，不会读写项目内数据文件。
